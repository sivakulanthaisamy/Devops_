pipeline {
    agent any
    environment {
        DOCKER_IMAGE = "siva182004/docker-app:latest"  // Change this to your registry
        CONTAINER_NAME = "docker-running-app"
        REGISTRY_CREDENTIALS = "docker-101"  // Jenkins credentials ID
    }
        ...
        stage('Checkout Code') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'git_101', usernameVariable: 'GIT_USER', passwordVariable: 'GIT_TOKEN')]) {
                    git url: "https://$GIT_USER:$GIT_TOKEN@github.com/sivakulanthaisamy/Docker_.git", branch: 'main'
                }
            }
        }
